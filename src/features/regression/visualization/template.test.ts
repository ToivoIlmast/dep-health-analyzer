import { buildRegressionHtmlTemplate, getCrossBoundaryConcentration } from './template';
import type { DependencyInsight } from '../types';

describe('getCrossBoundaryConcentration', () => {
    it('should return Low when there are no findings at all', () => {
        expect(getCrossBoundaryConcentration({ crossBoundaryCount: 0, totalFindings: 0 })).toBe(
            'Low Cross-Boundary Concentration'
        );
    });

    it('should return Low when the cross-boundary share is at or below 5%', () => {
        expect(getCrossBoundaryConcentration({ crossBoundaryCount: 5, totalFindings: 100 })).toBe(
            'Low Cross-Boundary Concentration'
        );
    });

    it('should return Moderate when the cross-boundary share is above 5% and at or below 15%', () => {
        expect(getCrossBoundaryConcentration({ crossBoundaryCount: 6, totalFindings: 100 })).toBe(
            'Moderate Cross-Boundary Concentration'
        );
        expect(getCrossBoundaryConcentration({ crossBoundaryCount: 15, totalFindings: 100 })).toBe(
            'Moderate Cross-Boundary Concentration'
        );
    });

    it('should return High when the cross-boundary share is above 15%', () => {
        expect(getCrossBoundaryConcentration({ crossBoundaryCount: 16, totalFindings: 100 })).toBe(
    ...
    it('should reproduce the real dep-health self-analysis case that motivated this fix', () => {
        ...
        const previouslyMisleadingResult = getCrossBoundaryConcentration({
            crossBoundaryCount: 4,
            totalFindings: 26,
        });

        expect(previouslyMisleadingResult).not.toBe('Low Cross-Boundary Concentration');
        expect(previouslyMisleadingResult).toBe('High Cross-Boundary Concentration');
    });
    ...
    describe('minimum cross-boundary count before percentage applies', () => {
        ...
        it('should stay Low with a single cross-boundary finding, no matter how small the change', () => {
            ...
            expect(getCrossBoundaryConcentration({ crossBoundaryCount: 1, totalFindings: 1 })).toBe(
                'Low Cross-Boundary Concentration'
            );

            // real case: 1 cross-boundary out of 3 total findings -> 33%
            expect(getCrossBoundaryConcentration({ crossBoundaryCount: 1, totalFindings: 3 })).toBe(
                'Low Cross-Boundary Concentration'
            );
        });
        ...
        it('should let percentage apply once there are at least 2 cross-boundary findings', () => {
            ...
            expect(getCrossBoundaryConcentration({ crossBoundaryCount: 9, totalFindings: 20 })).toBe(
                'High Cross-Boundary Concentration'
            );

            expect(getCrossBoundaryConcentration({ crossBoundaryCount: 2, totalFindings: 100 })).toBe(
                'Low Cross-Boundary Concentration'
            );
        });
    });
