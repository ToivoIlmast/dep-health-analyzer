...
export interface Dictionary {
    ...
    // --- Explore SCC / What-if modal (F25b) ------------------------------
    // A separate dialog from the concrete cycle modal above - it answers a
    // different question (the SCC's own structure as a whole, and the
    // precise, computed consequence of removing one chosen internal edge),
    // never "the cycle". Every string here is a neutral fact: no ranking,
    // no "important"/"best"/"representative" edge, no recommendation to
    // act. Plain "Label: %n" substitution throughout (formatI18n/
    // formatI18nClient, the same mechanism every other key already uses) -
    // deliberately no new pluralization system (F23, ru/pl/ar's
    // count-dependent plural classes, stays open/unrelated).
    exploreSccButton: string; // 'Explore SCC'
    exploreInternalEdgesLabel: string; // 'Internal dependencies: %n'
    exploreSingleCycleFact: string; // 'This group is a single cycle through all %n modules.'
    exploreMultiCycleFact: string; // 'This group contains more than one cycle. The exact number is not counted.'
    exploreModulesHeading: string; // 'Modules'
    whatIfToggleLabel: string; // 'What if one internal dependency were removed?'
    whatIfFromLabel: string; // 'From:'
    whatIfToLabel: string; // 'To:'
    whatIfOutcomeUnchanged: string; // 'Unchanged: all %n modules remain mutually dependent through another path.'
    whatIfOutcomeReduced: string; // 'Reduced: %n modules remain mutually dependent; %hidden module(s) would no longer be in any cycle here.'
    whatIfOutcomeSplit: string; // 'Split: this group would separate into %n independent cycle groups.'
    whatIfOutcomeAcyclic: string; // 'Acyclic: none of these %n modules would remain in any cycle.'
    whatIfRemainingGroupsHeading: string; // 'Remaining cycle groups:'
    whatIfNoLongerHeading: string; // 'No longer in any cycle: %n'
    whatIfProjectModulesInCycles: string; // 'Modules in cycles (whole project): %before &rarr; %after.'
    whatIfDisclaimer: string; // 'Hypothetical: computed on the analyzed graph with this dependency removed. Source code is not changed. This is not a recommendation.'
...
export const I18N: Record<LanguageCode, Dictionary> = {
    en: {
        ...
        closeButton: 'Close',
        exploreSccButton: 'Explore SCC',
        exploreInternalEdgesLabel: 'Internal dependencies: %n',
        exploreSingleCycleFact: 'This group is a single cycle through all %n modules.',
        exploreMultiCycleFact: 'This group contains more than one cycle. The exact number is not counted.',
        exploreModulesHeading: 'Modules',
        whatIfToggleLabel: 'What if one internal dependency were removed?',
        whatIfFromLabel: 'From:',
        whatIfToLabel: 'To:',
        whatIfOutcomeUnchanged: 'Unchanged: all %n modules remain mutually dependent through another path.',
        whatIfOutcomeReduced: 'Reduced: %n modules remain mutually dependent; %hidden module(s) would no longer be in any cycle here.',
        whatIfOutcomeSplit: 'Split: this group would separate into %n independent cycle groups.',
        whatIfOutcomeAcyclic: 'Acyclic: none of these %n modules would remain in any cycle.',
        whatIfRemainingGroupsHeading: 'Remaining cycle groups:',
        whatIfNoLongerHeading: 'No longer in any cycle: %n',
        whatIfProjectModulesInCycles: 'Modules in cycles (whole project): %before &rarr; %after.',
        whatIfDisclaimer: 'Hypothetical: computed on the analyzed graph with this dependency removed. Source code is not changed. This is not a recommendation.',
    ...
    // Findings strings (riippuvuus=dependency, moduuli=module, etc.).
        ...
        closeButton: 'Sulje',
        exploreSccButton: 'Tarkastele SCC:tä',
        exploreInternalEdgesLabel: 'Sisäiset riippuvuudet: %n',
        exploreSingleCycleFact: 'Tämä ryhmä on yksi sykli, joka kulkee kaikkien %n moduulin kautta.',
        exploreMultiCycleFact: 'Tämä ryhmä sisältää useamman kuin yhden syklin. Tarkkaa määrää ei lasketa.',
        exploreModulesHeading: 'Moduulit',
        whatIfToggleLabel: 'Entä jos yksi sisäinen riippuvuus poistettaisiin?',
        whatIfFromLabel: 'Mistä:',
        whatIfToLabel: 'Mihin:',
        whatIfOutcomeUnchanged: 'Ei muutosta: kaikki %n moduulia pysyvät keskinäisesti riippuvaisina toista reittiä pitkin.',
        whatIfOutcomeReduced: 'Pienenee: %n moduulia pysyy keskinäisesti riippuvaisena; %hidden moduuli(a) ei enää kuuluisi mihinkään sykliin täällä.',
        whatIfOutcomeSplit: 'Jakautuu: tämä ryhmä jakautuisi %n toisistaan riippumattomaan syklien ryhmään.',
        whatIfOutcomeAcyclic: 'Syklitön: yksikään näistä %n moduulista ei enää kuuluisi mihinkään sykliin.',
        whatIfRemainingGroupsHeading: 'Jäljelle jäävät syklien ryhmät:',
        whatIfNoLongerHeading: 'Ei enää missään syklissä: %n',
        whatIfProjectModulesInCycles: 'Moduuleja sykleissä (koko projekti): %before &rarr; %after.',
        whatIfDisclaimer: 'Hypoteettinen: laskettu analysoidusta graafista tämän riippuvuuden poistamisen jälkeen. Lähdekoodia ei muuteta. Tämä ei ole suositus.',
    ...
    sv: {
        ...
        closeButton: 'Stäng',
        exploreSccButton: 'Utforska SCC',
        exploreInternalEdgesLabel: 'Interna beroenden: %n',
        exploreSingleCycleFact: 'Denna grupp är en enda cykel genom alla %n moduler.',
        exploreMultiCycleFact: 'Denna grupp innehåller mer än en cykel. Det exakta antalet räknas inte.',
        exploreModulesHeading: 'Moduler',
        whatIfToggleLabel: 'Vad händer om ett internt beroende togs bort?',
        whatIfFromLabel: 'Från:',
        whatIfToLabel: 'Till:',
        whatIfOutcomeUnchanged: 'Oförändrad: alla %n moduler förblir ömsesidigt beroende via en annan väg.',
        whatIfOutcomeReduced: 'Minskad: %n moduler förblir ömsesidigt beroende; %hidden modul(er) skulle inte längre ingå i någon cykel här.',
        whatIfOutcomeSplit: 'Delad: denna grupp skulle delas upp i %n oberoende cykelgrupper.',
        whatIfOutcomeAcyclic: 'Cykelfri: ingen av dessa %n moduler skulle längre ingå i någon cykel.',
        whatIfRemainingGroupsHeading: 'Kvarvarande cykelgrupper:',
        whatIfNoLongerHeading: 'Inte längre i någon cykel: %n',
        whatIfProjectModulesInCycles: 'Moduler i cykler (hela projektet): %before &rarr; %after.',
        whatIfDisclaimer: 'Hypotetiskt: beräknat på den analyserade grafen med detta beroende borttaget. Källkoden ändras inte. Detta är ingen rekommendation.',
    ...
    no: {
        ...
        closeButton: 'Lukk',
        exploreSccButton: 'Utforsk SCC',
        exploreInternalEdgesLabel: 'Interne avhengigheter: %n',
        exploreSingleCycleFact: 'Denne gruppen er én enkelt syklus gjennom alle %n modulene.',
        exploreMultiCycleFact: 'Denne gruppen inneholder mer enn én syklus. Det nøyaktige antallet telles ikke.',
        exploreModulesHeading: 'Moduler',
        whatIfToggleLabel: 'Hva om én intern avhengighet ble fjernet?',
        whatIfFromLabel: 'Fra:',
        whatIfToLabel: 'Til:',
        whatIfOutcomeUnchanged: 'Uendret: alle %n moduler forblir gjensidig avhengige via en annen vei.',
        whatIfOutcomeReduced: 'Redusert: %n moduler forblir gjensidig avhengige; %hidden modul(er) ville ikke lenger vært del av noen syklus her.',
        whatIfOutcomeSplit: 'Delt: denne gruppen ville deles i %n uavhengige sykelgrupper.',
        whatIfOutcomeAcyclic: 'Sykelfri: ingen av disse %n modulene ville lenger vært del av noen syklus.',
        whatIfRemainingGroupsHeading: 'Gjenværende sykelgrupper:',
        whatIfNoLongerHeading: 'Ikke lenger i noen syklus: %n',
        whatIfProjectModulesInCycles: 'Moduler i sykluser (hele prosjektet): %before &rarr; %after.',
        whatIfDisclaimer: 'Hypotetisk: beregnet på den analyserte grafen med denne avhengigheten fjernet. Kildekoden endres ikke. Dette er ingen anbefaling.',
    ...
    da: {
        ...
        closeButton: 'Luk',
        exploreSccButton: 'Udforsk SCC',
        exploreInternalEdgesLabel: 'Interne afhængigheder: %n',
        exploreSingleCycleFact: 'Denne gruppe er én enkelt cyklus gennem alle %n moduler.',
        exploreMultiCycleFact: 'Denne gruppe indeholder mere end én cyklus. Det nøjagtige antal tælles ikke.',
        exploreModulesHeading: 'Moduler',
        whatIfToggleLabel: 'Hvad hvis én intern afhængighed blev fjernet?',
        whatIfFromLabel: 'Fra:',
        whatIfToLabel: 'Til:',
        whatIfOutcomeUnchanged: 'Uændret: alle %n moduler forbliver gensidigt afhængige via en anden vej.',
        whatIfOutcomeReduced: 'Reduceret: %n moduler forbliver gensidigt afhængige; %hidden modul(er) ville ikke længere indgå i nogen cyklus her.',
        whatIfOutcomeSplit: 'Opdelt: denne gruppe ville opdeles i %n uafhængige cyklusgrupper.',
        whatIfOutcomeAcyclic: 'Cyklusfri: ingen af disse %n moduler ville længere indgå i nogen cyklus.',
        whatIfRemainingGroupsHeading: 'Resterende cyklusgrupper:',
        whatIfNoLongerHeading: 'Ikke længere i nogen cyklus: %n',
        whatIfProjectModulesInCycles: 'Moduler i cyklusser (hele projektet): %before &rarr; %after.',
        whatIfDisclaimer: 'Hypotetisk: beregnet på den analyserede graf med denne afhængighed fjernet. Kildekoden ændres ikke. Dette er ikke en anbefaling.',
    ...
    is: {
        ...
        closeButton: 'Loka',
        exploreSccButton: 'Skoða SCC',
        exploreInternalEdgesLabel: 'Innri ósjálfstæði: %n',
        exploreSingleCycleFact: 'Þessi hópur er ein hringrás í gegnum allar %n einingar.',
        exploreMultiCycleFact: 'Þessi hópur inniheldur fleiri en eina hringrás. Nákvæmur fjöldi er ekki talinn.',
        exploreModulesHeading: 'Einingar',
        whatIfToggleLabel: 'Hvað ef eitt innra ósjálfstæði væri fjarlægt?',
        whatIfFromLabel: 'Frá:',
        whatIfToLabel: 'Til:',
        whatIfOutcomeUnchanged: 'Óbreytt: allar %n einingar eru áfram gagnkvæmt háðar í gegnum aðra leið.',
        whatIfOutcomeReduced: 'Minnkað: %n einingar eru áfram gagnkvæmt háðar; %hidden eining(ar) væru ekki lengur hluti af neinni hringrás hér.',
        whatIfOutcomeSplit: 'Skipt: þessi hópur myndi skiptast í %n óháða hringrásarhópa.',
        whatIfOutcomeAcyclic: 'Hringrásarlaust: engin af þessum %n einingum væri lengur hluti af hringrás.',
        whatIfRemainingGroupsHeading: 'Eftirstandandi hringrásarhópar:',
        whatIfNoLongerHeading: 'Ekki lengur í neinni hringrás: %n',
        whatIfProjectModulesInCycles: 'Einingar í hringrásum (allt verkefnið): %before &rarr; %after.',
        whatIfDisclaimer: 'Ímyndað: reiknað út frá greindu grafinu með þessu ósjálfstæði fjarlægðu. Frumkóða er ekki breytt. Þetta er ekki tilmæli.',
    ...
    de: {
        ...
        exploreSccButton: 'SCC erkunden',
        exploreInternalEdgesLabel: 'Interne Abhängigkeiten: %n',
        exploreSingleCycleFact: 'Diese Gruppe ist ein einziger Zyklus durch alle %n Module.',
        exploreMultiCycleFact: 'Diese Gruppe enthält mehr als einen Zyklus. Die genaue Anzahl wird nicht gezählt.',
        exploreModulesHeading: 'Module',
        whatIfToggleLabel: 'Was, wenn eine interne Abhängigkeit entfernt würde?',
        whatIfFromLabel: 'Von:',
        whatIfToLabel: 'Nach:',
        whatIfOutcomeUnchanged: 'Unverändert: alle %n Module bleiben über einen anderen Pfad gegenseitig abhängig.',
        whatIfOutcomeReduced: 'Verkleinert: %n Module bleiben gegenseitig abhängig; %hidden Modul(e) wären hier nicht mehr Teil eines Zyklus.',
        whatIfOutcomeSplit: 'Aufgeteilt: diese Gruppe würde sich in %n unabhängige Zyklusgruppen aufteilen.',
        whatIfOutcomeAcyclic: 'Azyklisch: keines dieser %n Module wäre noch Teil eines Zyklus.',
        whatIfRemainingGroupsHeading: 'Verbleibende Zyklusgruppen:',
        whatIfNoLongerHeading: 'Nicht mehr Teil eines Zyklus: %n',
        whatIfProjectModulesInCycles: 'Module in Zyklen (gesamtes Projekt): %before &rarr; %after.',
        whatIfDisclaimer: 'Hypothetisch: berechnet auf dem analysierten Graphen mit dieser entfernten Abhängigkeit. Der Quellcode wird nicht geändert. Dies ist keine Empfehlung.',
    ...
    fr: {
        ...
        closeButton: 'Fermer',
        exploreSccButton: 'Explorer le SCC',
        exploreInternalEdgesLabel: 'Dépendances internes : %n',
        exploreSingleCycleFact: 'Ce groupe est un cycle unique passant par les %n modules.',
        exploreMultiCycleFact: "Ce groupe contient plus d'un cycle. Le nombre exact n'est pas compté.",
        exploreModulesHeading: 'Modules',
        whatIfToggleLabel: 'Et si une dépendance interne était supprimée ?',
        whatIfFromLabel: 'De :',
        whatIfToLabel: 'Vers :',
        whatIfOutcomeUnchanged: 'Inchangé : les %n modules restent mutuellement dépendants via un autre chemin.',
        whatIfOutcomeReduced: "Réduit : %n modules restent mutuellement dépendants ; %hidden module(s) ne ferai(en)t plus partie d'aucun cycle ici.",
        whatIfOutcomeSplit: 'Scindé : ce groupe se diviserait en %n groupes de cycles indépendants.',
        whatIfOutcomeAcyclic: "Acyclique : aucun de ces %n modules ne ferait plus partie d'un cycle.",
        whatIfRemainingGroupsHeading: 'Groupes de cycles restants :',
        whatIfNoLongerHeading: "Ne fait plus partie d'un cycle : %n",
        whatIfProjectModulesInCycles: 'Modules dans des cycles (projet entier) : %before &rarr; %after.',
        whatIfDisclaimer: "Hypothétique : calculé sur le graphe analysé avec cette dépendance supprimée. Le code source n'est pas modifié. Ce n'est pas une recommandation.",
    ...
    es: {
        ...
        closeButton: 'Cerrar',
        exploreSccButton: 'Explorar SCC',
        exploreInternalEdgesLabel: 'Dependencias internas: %n',
        exploreSingleCycleFact: 'Este grupo es un único ciclo que pasa por los %n módulos.',
        exploreMultiCycleFact: 'Este grupo contiene más de un ciclo. El número exacto no se cuenta.',
        exploreModulesHeading: 'Módulos',
        whatIfToggleLabel: '¿Qué pasaría si se eliminara una dependencia interna?',
        whatIfFromLabel: 'Desde:',
        whatIfToLabel: 'Hasta:',
        whatIfOutcomeUnchanged: 'Sin cambios: los %n módulos siguen siendo mutuamente dependientes a través de otra ruta.',
        whatIfOutcomeReduced: 'Reducido: %n módulos siguen siendo mutuamente dependientes; %hidden módulo(s) dejarían de formar parte de cualquier ciclo aquí.',
        whatIfOutcomeSplit: 'Dividido: este grupo se dividiría en %n grupos de ciclos independientes.',
        whatIfOutcomeAcyclic: 'Acíclico: ninguno de estos %n módulos seguiría formando parte de un ciclo.',
        whatIfRemainingGroupsHeading: 'Grupos de ciclos restantes:',
        whatIfNoLongerHeading: 'Ya no forma parte de ningún ciclo: %n',
        whatIfProjectModulesInCycles: 'Módulos en ciclos (todo el proyecto): %before &rarr; %after.',
        whatIfDisclaimer: 'Hipotético: calculado sobre el grafo analizado con esta dependencia eliminada. El código fuente no se modifica. Esto no es una recomendación.',
    ...
    pl: {
        ...
        closeButton: 'Zamknij',
        exploreSccButton: 'Eksploruj SCC',
        exploreInternalEdgesLabel: 'Zależności wewnętrzne: %n',
        exploreSingleCycleFact: 'Ta grupa to pojedynczy cykl przechodzący przez wszystkie %n modułów.',
        exploreMultiCycleFact: 'Ta grupa zawiera więcej niż jeden cykl. Dokładna liczba nie jest liczona.',
        exploreModulesHeading: 'Moduły',
        whatIfToggleLabel: 'Co by było, gdyby usunięto jedną wewnętrzną zależność?',
        whatIfFromLabel: 'Od:',
        whatIfToLabel: 'Do:',
        whatIfOutcomeUnchanged: 'Bez zmian: wszystkie %n modułów pozostaje wzajemnie zależnych poprzez inną ścieżkę.',
        whatIfOutcomeReduced: 'Zmniejszona: %n modułów pozostaje wzajemnie zależnych; %hidden moduł(y) nie należał(yby) już do żadnego cyklu w tym miejscu.',
        whatIfOutcomeSplit: 'Podzielona: ta grupa podzieliłaby się na %n niezależnych grup cykli.',
        whatIfOutcomeAcyclic: 'Acykliczna: żaden z tych %n modułów nie należałby już do żadnego cyklu.',
        whatIfRemainingGroupsHeading: 'Pozostałe grupy cykli:',
        whatIfNoLongerHeading: 'Już nie w żadnym cyklu: %n',
        whatIfProjectModulesInCycles: 'Moduły w cyklach (cały projekt): %before &rarr; %after.',
        whatIfDisclaimer: 'Hipotetycznie: obliczone na przeanalizowanym grafie po usunięciu tej zależności. Kod źródłowy nie jest zmieniany. To nie jest rekomendacja.',
    ...
    pt: {
        ...
        closeButton: 'Fechar',
        exploreSccButton: 'Explorar SCC',
        exploreInternalEdgesLabel: 'Dependências internas: %n',
        exploreSingleCycleFact: 'Este grupo é um único ciclo que passa por todos os %n módulos.',
        exploreMultiCycleFact: 'Este grupo contém mais do que um ciclo. O número exato não é contado.',
        exploreModulesHeading: 'Módulos',
        whatIfToggleLabel: 'E se uma dependência interna fosse removida?',
        whatIfFromLabel: 'De:',
        whatIfToLabel: 'Para:',
        whatIfOutcomeUnchanged: 'Inalterado: todos os %n módulos permanecem mutuamente dependentes através de outro caminho.',
        whatIfOutcomeReduced: 'Reduzido: %n módulos permanecem mutuamente dependentes; %hidden módulo(s) deixaria(m) de fazer parte de qualquer ciclo aqui.',
        whatIfOutcomeSplit: 'Dividido: este grupo dividir-se-ia em %n grupos de ciclos independentes.',
        whatIfOutcomeAcyclic: 'Acíclico: nenhum destes %n módulos continuaria a fazer parte de um ciclo.',
        whatIfRemainingGroupsHeading: 'Grupos de ciclos restantes:',
        whatIfNoLongerHeading: 'Já não faz parte de nenhum ciclo: %n',
        whatIfProjectModulesInCycles: 'Módulos em ciclos (projeto inteiro): %before &rarr; %after.',
        whatIfDisclaimer: 'Hipotético: calculado no grafo analisado com esta dependência removida. O código-fonte não é alterado. Isto não é uma recomendação.',
    ...
    ru: {
        ...
        closeButton: 'Закрыть',
        exploreSccButton: 'Исследовать SCC',
        exploreInternalEdgesLabel: 'Внутренние зависимости: %n',
        exploreSingleCycleFact: 'Эта группа — единый цикл, проходящий через все %n модулей.',
        exploreMultiCycleFact: 'Эта группа содержит более одного цикла. Точное количество не подсчитывается.',
        exploreModulesHeading: 'Модули',
        whatIfToggleLabel: 'Что если удалить одну внутреннюю зависимость?',
        whatIfFromLabel: 'Откуда:',
        whatIfToLabel: 'Куда:',
        whatIfOutcomeUnchanged: 'Без изменений: все %n модулей остаются взаимозависимыми через другой путь.',
        whatIfOutcomeReduced: 'Уменьшилась: %n модулей остаются взаимозависимыми; %hidden модуль(ей) больше не входили бы ни в один цикл здесь.',
        whatIfOutcomeSplit: 'Распалась: эта группа разделилась бы на %n независимых групп циклов.',
        whatIfOutcomeAcyclic: 'Ацикличность: ни один из этих %n модулей больше не входил бы ни в один цикл.',
        whatIfRemainingGroupsHeading: 'Оставшиеся группы циклов:',
        whatIfNoLongerHeading: 'Больше не в цикле: %n',
        whatIfProjectModulesInCycles: 'Модулей в циклах (весь проект): %before &rarr; %after.',
        whatIfDisclaimer: 'Гипотетически: рассчитано на проанализированном графе с удалённой зависимостью. Исходный код не изменяется. Это не рекомендация.',
    ...
    ar: {
        ...
        closeButton: 'إغلاق',
        exploreSccButton: 'استكشاف SCC',
        exploreInternalEdgesLabel: 'التبعيات الداخلية: %n',
        exploreSingleCycleFact: 'هذه المجموعة عبارة عن دورة واحدة تمر عبر جميع الوحدات البالغ عددها %n.',
        exploreMultiCycleFact: 'تحتوي هذه المجموعة على أكثر من دورة واحدة. لا يتم احتساب العدد الدقيق.',
        exploreModulesHeading: 'الوحدات',
        whatIfToggleLabel: 'ماذا لو تمت إزالة تبعية داخلية واحدة؟',
        whatIfFromLabel: 'من:',
        whatIfToLabel: 'إلى:',
        whatIfOutcomeUnchanged: 'بلا تغيير: تظل جميع الوحدات البالغ عددها %n مترابطة عبر مسار آخر.',
        whatIfOutcomeReduced: 'تقلصت: تظل %n وحدة مترابطة؛ لن تعود %hidden وحدة (وحدات) ضمن أي دورة هنا.',
        whatIfOutcomeSplit: 'انقسمت: ستنقسم هذه المجموعة إلى %n مجموعات دورات مستقلة.',
        whatIfOutcomeAcyclic: 'غير دوري: لن تبقى أي من هذه الوحدات البالغ عددها %n ضمن أي دورة.',
        whatIfRemainingGroupsHeading: 'مجموعات الدورات المتبقية:',
        whatIfNoLongerHeading: 'لم تعد ضمن أي دورة: %n',
        whatIfProjectModulesInCycles: 'الوحدات ضمن دورات (المشروع بأكمله): %before &rarr; %after.',
        whatIfDisclaimer: 'افتراضي: محسوب على الرسم البياني الذي تم تحليله بعد إزالة هذه التبعية. لا يتم تغيير الشيفرة المصدرية. هذه ليست توصية.',
    ...
    ja: {
        ...
        closeButton: '閉じる',
        exploreSccButton: 'SCC を調べる',
        exploreInternalEdgesLabel: '内部依存関係: %n',
        exploreSingleCycleFact: 'このグループは %n 個すべてのモジュールを通る単一の循環です。',
        exploreMultiCycleFact: 'このグループには複数の循環が含まれています。正確な数は数えていません。',
        exploreModulesHeading: 'モジュール',
        whatIfToggleLabel: '内部依存関係を1つ削除するとどうなりますか?',
        whatIfFromLabel: '依存元:',
        whatIfToLabel: '依存先:',
        whatIfOutcomeUnchanged: '変化なし: %n 個のモジュールは別の経路を通じて相互に依存したままです。',
        whatIfOutcomeReduced: '縮小: %n 個のモジュールが相互依存を維持しますが、%hidden 個のモジュールはここでどの循環にも含まれなくなります。',
        whatIfOutcomeSplit: '分割: このグループは %n 個の独立した循環グループに分かれます。',
        whatIfOutcomeAcyclic: '非循環化: これら %n 個のモジュールはいずれもどの循環にも含まれなくなります。',
        whatIfRemainingGroupsHeading: '残る循環グループ:',
        whatIfNoLongerHeading: 'もうどの循環にも含まれない: %n',
        whatIfProjectModulesInCycles: '循環に含まれるモジュール数(プロジェクト全体): %before &rarr; %after。',
        whatIfDisclaimer: '仮定: この依存関係を削除した状態で解析済みのグラフ上で計算しています。ソースコードは変更されません。これは推奨ではありません。',
